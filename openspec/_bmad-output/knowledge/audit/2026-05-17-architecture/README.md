...
| `cell-occupancy.md` | Per-cluster cell labels (42 clusters → 15 cells), per-cell saturation status (93% strict / 80% inclusive K=2), QD-score baseline (~58.8 sum of max σ × κ), path-dependence inventory (60% Wave-1 dominance). **SUPERSEDED by `wave-8-path-dependence-results.md` §3** for current saturation + cluster counts; preserved for historical reference. |
| `wave-8-path-dependence-results.md` | Wave-8 path-dependence experiment (Option C): 3 streams adding second-methods in cells with only one prior method opened ~30 NEW clusters, expanding 42 → 72 clusters and dropping saturation 93% → 58%. Empirically validates `methodology-research-note.md` §2.2 path-dependence concern; documentation-only mitigation insufficient. 4 methodology implications + 3 updated stopping options (A' continue / B' accept incomplete / C' bootstrap restart). |
| `closure-argument-framework.md` | **The framework spec** (audit-type-agnostic). 7-step closure-argument framework: separate behavioural descriptors / quality predicates / audit-type weights; argue closure at axis level (not cell level); trunk + frontier with promotion; three convergence signals (inter-perspective clustering + mutation catch + information gain); archetype priors per audit-type; contextual-bandit adaptive depth; semantic-versioned matrix. **Supersedes `methodology-research-note.md` §3** (saturation-based reformulation was partial; this subsumes). Research reconciliation with Pugh 2016 / Mouret & Clune 2015 / Cully 2018 / Settles 2009 / DeMillo 1978 / MacKay 1992 / Auer 2002 / Pierrot 2022 / Stock 2025 + adversarial-ML + RLHF analogues. POC instantiation in TASK-0053. |
