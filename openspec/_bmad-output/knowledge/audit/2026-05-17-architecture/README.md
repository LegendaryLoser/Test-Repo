...
| `cell-occupancy.md` | Per-cluster cell labels (42 clusters → 15 cells), per-cell saturation status (93% strict / 80% inclusive K=2), QD-score baseline (~58.8 sum of max σ × κ), path-dependence inventory (60% Wave-1 dominance). Audit close to cluster-level saturation; depth still expanding (W7 ΔQD-score 29%). Empirical foundation for saturation-based stopping. |
